...
if ($_SESSION['User'] != NULL)
{
    ...
    $sql23 = "SELECT * FROM orders O WHERE O.status ='Pending'";
	$result23 = mysqli_query($conn,$sql23);
    $posts23 = mysqli_fetch_all($result23,MYSQLI_ASSOC);
    
    if (isset($_POST['chg_order']))
    {
        $sql33 = "SELECT * FROM orders O WHERE O.status != 'Delivered'";
        $result33 = mysqli_query($conn,$sql33);
        $posts33 = mysqli_fetch_all($result33,MYSQLI_ASSOC);
    }
    else 
    {
        $sql33 = "SELECT * FROM orders O WHERE O.status != 'Delivered'";
...
<!-- 
            ...
            show order id and choose to recieved 
            status = recieved  DONE 


            Update an Order 
            to making and stuff

            cancel order make the status cancel .

    -->
    <section class="ftco-section contact-section">
            <h2 class="text-md-center">Staff Options</h2>
      <div class="container mt-5">
        <div class="row block-9">
            <form action="staff_side.php" method="post">
					<div class="col-md-6 contact-info ftco-animate">
						<div class="row">
                        <p style="font-size:20px; color:#fff;" ><span style="font-size:20px;" ></span> Receive an Order : </p>
                             <select name="order_tb_rec" style="width:230px;">
                                <option value="" disabled selected hidden>Please Choose An Order ... </option>
                                <?php  foreach($posts23 as $key): ?>
                                    <option value="<?php echo $key['orderId'];?>"> <?php echo $key['orderId']; ?></option>
                                        <?php endforeach; ?>
                                </select> 
                            <div class="form-group pt-5" style="justify-content: center;">
								<button type="submit"  name="rec_order"  class="btn btn-primary py-3 px-5"> Submit </button>
							</div>
                        </div>
            </form> 
         </div>     
          <div class="row block-9">
            <form action="staff_side.php" method="post">
					<div class="col-md-6 contact-info ftco-animate">
						<div class="row">
                        <p style="font-size:20px; color:#fff;" ><span style="font-size:20px;" ></span> Cancel an Order : </p>
                             <select name="order_tb_cnc" style="width:230px;">
                                <option value="" disabled selected hidden>Please Choose An Order ... </option>
                                <?php  foreach($posts33 as $key2): ?>
                                    <option value="<?php echo $key2['orderId'];?>"> <?php echo $key2['orderId']; ?></option>
                                        <?php endforeach; ?>
                                </select> 
                            <div class="form-group pt-5" style="justify-content: center;">
								<button type="submit"  name="cnn_order"  class="btn btn-primary py-3 px-5"> Submit </button>
							</div>
                        </div>
            </form> 
         </div>    
                ...
                <div class="row block-9">
                    <form action="staff_side.php" method="post">
                            <div class="col-md-6 contact-info ftco-animate">
                                ...
                                <p style="font-size:20px; color:#fff;" ><span style="font-size:20px;" ></span> Receive an Order : </p>
                                     <select name="order_tb_num" style="width:230px;" required="required">
                                        <option value="" disabled selected hidden>Please Choose An Order ... </option>
                                        <?php  foreach($posts33 as $key2): ?>
                                            <option value="<?php echo $key2['orderId'];?>"> <?php echo $key2['orderId']; ?></option>
                                                <?php endforeach; ?>
                                    ...
                                    <div class="row" style="padding-top:25px;">
                                        <select name="order_tb_new" style="width:230px;" required="required" >
                                            <option value="" disabled selected hidden>Please Choose An Order ... </option>
                                    ...
                                    <div class="form-group pt-5" style="justify-content: center;">
                                        <button type="submit"  name="chg_order"  class="btn btn-primary py-3 px-5"> Submit </button>
                                    </div>
                    </form> 
                 </div>  
                    <!--  -->
